...
async def recipes() -> List[models_rb.Recipe]:
    res = await session.execute(select(models_rb.Recipe).order_by(models_rb.Recipe.views_number.desc(),
...
async def recipes(idx: int) -> models_rb.Recipe:
    cur_views = await session.execute(select(models_rb.Recipe.views_number).filter_by(id=idx))
    views_num = cur_views.scalar_one_or_none()
    if views_num is None:
        raise HTTPException(status_code=404, detail="Recipe not found")
    new_views = views_num + 1
    await session.execute(update(models_rb.Recipe).filter_by(id=idx).values(views_number=new_views))
